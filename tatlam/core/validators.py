...
def validate_scenario_doctrine(scenario: dict[str, Any]) -> DoctrineValidationResult:
    ...
    if threat:
        ...
        threat_normalized = (
            threat.replace("נמוכה", "LOW")
            .replace("בינונית", "MEDIUM")
            .replace("גבוהה", "HIGH")
            .replace("קריטית", "CRITICAL")
        )
    ...
    result = DoctrineValidationResult(
        is_valid=len(errors) == 0, errors=errors, warnings=warnings, doctrine_score=score
    )

    logger.debug(
        "Doctrine validation: valid=%s, score=%d, errors=%d, warnings=%d",
        result.is_valid,
        result.doctrine_score,
        len(errors),
        len(warnings),
    )
