...
class ScenarioStatus(str, Enum):
    ...
    APPROVED = "approved"    # Passed Judge threshold
...
class SwarmState:
    ...
    # === Mission Scope ===
    bundle_id: str = field(default_factory=lambda: f"BUNDLE-{datetime.now().strftime('%Y%m%d-%H%M')}")
    category: str = ""
    target_count: int = 5
    ...
    def approved_scenarios(self) -> list[ScenarioCandidate]:
        """Scenarios that passed the Judge."""
        return [c for c in self.candidates if c.status == ScenarioStatus.APPROVED]
    ...
    def pending_scenarios(self) -> list[ScenarioCandidate]:
        ...
        return [c for c in self.candidates
                if c.status not in (ScenarioStatus.APPROVED, ScenarioStatus.REJECTED, ScenarioStatus.ARCHIVED)]
    ...
    def log_phase_change(self, new_phase: WorkflowPhase) -> None:
        ...
        logger.info(
            "Phase transition: %s → %s (iteration=%d, approved=%d/%d)",
            old_phase.value, new_phase.value,
            self.iteration, len(self.approved_scenarios), self.target_count
        )
