...
class TestUtils:
    ...
    def test_normalize_row_with_sqlite_row(self):
        ...
        mock_row = MockRow({"id": 1, "title": "כותרת בדיקה", "category": "פיננסים"})
    ...
    def test_normalize_row_preserves_hebrew(self):
        ...
        hebrew_data = {"title": "בדיקת עברית מלאה", "description": "תיאור בעברית עם ניקוד: שָׁלוֹם"}
    ...
    def test_normalize_row_handles_null_values(self):
        ...
        mock_row = MockRow({"id": 1, "title": "כותרת", "optional_field": None})
    ...
    def test_normalize_row_with_json_fields(self):
        ...
        steps_json = json.dumps(
            [{"step": 1, "description": "צעד ראשון"}, {"step": 2, "description": "צעד שני"}]
        )

        mock_row = MockRow({"id": 1, "steps": steps_json})
